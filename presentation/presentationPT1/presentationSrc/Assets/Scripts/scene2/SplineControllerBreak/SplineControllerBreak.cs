...
[RequireComponent(typeof(SplineInterpolatorBreak))]

public class SplineControllerBreak : MonoBehaviour
{
	...
	SplineInterpolatorBreak mSplineInterp;
	...
	//   - BreakTime
	//   - Name
	SplineNodeBreak[] mSplineNodeInfo;
	...
	void OnDrawGizmos()
	{
		...
		SplineNodeBreak[] info = GetSplineNodes();
		if (info.Length < 2)
			return;

		SplineInterpolatorBreak interp = GetComponent(typeof(SplineInterpolatorBreak)) as SplineInterpolatorBreak;
		...
		for (int c = 0; c <= 100; c++)
		{
			...
			if (float.IsNaN(currPos.x))
				currPos.x = 0;//Debug.Log("NaN while drawing gizmos!!!!"); // should never arrive here!
	...
	void Start()
	{
		mSplineInterp = GetComponent(typeof(SplineInterpolatorBreak)) as SplineInterpolatorBreak;
	...
	float GetDuration(SplineNodeBreak[] info)
	{
		float endTime = -TimeBetweenAdjacentNodes;
		foreach (SplineNodeBreak e in info)
			endTime += TimeBetweenAdjacentNodes + e.BreakTime;
		
		return endTime;
	}
	
	void SetupSplineInterpolator(SplineInterpolatorBreak interp, SplineNodeBreak[] ninfo)
	{
		interp.Reset();
	...
	/// </summary>
	SplineNodeBreak[] GetSplineNodes()
	{
		if (SplineRoot == null)
			return null;
		...
		List<SplineNodeBreak> info = new List<SplineNodeBreak>();
		foreach (Transform element in transforms)
    	{
			SplineNodeProperties p = element.GetComponent<SplineNodeProperties>();
			if (p != null)
				info.Add(new SplineNodeBreak(p.Name, element.transform, p.BreakTime));
			else
				info.Add(new SplineNodeBreak("", element.transform, 0));
		}
