...
public class SplineNodeBreak
{
	...
	internal SplineNodeBreak(string n, Vector3 p, Quaternion q, float tArrival, float tBreak, Vector2 io) 
		{ Name = n; Point = p; Rot = q; ArrivalTime = tArrival; BreakTime = tBreak; EaseIO = io; }
	
	internal SplineNodeBreak(SplineNodeBreak o) 
	...
	internal SplineNodeBreak(string n, Transform t, float tBreak) 
...
public delegate void OnNodeArrivalCallback(int idxArrival, SplineNodeBreak nodeArrival);
public delegate void OnNodeLeavingCallback(int idxLeaving, SplineNodeBreak nodeLeaving);

public class SplineInterpolatorBreak : MonoBehaviour
{
	List<SplineNodeBreak> mNodes = new List<SplineNodeBreak>();
						 ...
						 Vector2 easeInOut)
	{
		...
		mNodes.Add(new SplineNodeBreak(name, pos, quat, timeInSeconds, timeStop, easeInOut));
	}
	...
	public void SetAutoCloseMode(float joiningPointTime)
	{
		...
		mNodes.Add(new SplineNodeBreak(mNodes[0] as SplineNodeBreak));
		...
		SplineNodeBreak firstNode = new SplineNodeBreak(mNodes[0] as SplineNodeBreak);
		firstNode.Point = mNodes[0].Point + vEndDir * firstLength;

		SplineNodeBreak lastNode = new SplineNodeBreak(mNodes[mNodes.Count - 1] as SplineNodeBreak);
	...
	void SetInput()
	{
		...
		if (mRotations)
		{
			...
			{
				SplineNodeBreak node = mNodes[c];
				SplineNodeBreak prevNode = mNodes[c - 1];
